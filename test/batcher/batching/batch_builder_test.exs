defmodule Batcher.Batching.BatchBuilderTest do
  ...
  describe "BatchBuilder lifecycle" do
    ...
    test "add_request/3 retries when BatchBuilder crashes between lookup and call" do
      ...
      [{new_pid, _}] = Registry.lookup(Batcher.Batching.Registry, {url, model})
    ...
    test "handles finish_building when batch not found" do
      ...
      pid = GenServer.whereis({:via, Registry, {Batcher.Batching.Registry, {url, model}}})
      ref = Process.monitor(pid)
