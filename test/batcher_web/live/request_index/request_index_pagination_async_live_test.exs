defmodule BatcherWeb.RequestIndexPaginationAsyncLiveTest do
  ...
  describe "pagination" do
    ...
    test "incoming requests stay buffered across passive patch reloads", %{
      ...
      {:ok, created_request} =
        Batching.create_request(%{
          batch_id: batch.id,
          custom_id: "buffered-during-patch",
          url: batch.url,
          model: batch.model,
          request_payload: %{
            custom_id: "buffered-during-patch",
      ...
      :timer.sleep(75)
      refute render(view) =~ "buffered-during-patch"
      assert has_element?(view, "button[phx-click='refresh_requests']", "Refresh")

      view
      |> element("a.join-item", "20")
      |> render_click()
      ...
      wait_for(fn -> render(view) =~ "buffered-during-patch" end)
    end
