defmodule Batcher.Batching.BatchBuilder do
  @moduledoc """
  ...
  BatchBuilders are registered in `Batcher.Batching.Registry` with key `{url, model}`.
  """
  ...
  def add_request(url, model, request_data, retries \\ 5) do
    case Registry.lookup(Batcher.Batching.Registry, {url, model}) do
      ...
      [] ->
        ...
        result =
          DynamicSupervisor.start_child(Batcher.Batching.Supervisor, {__MODULE__, {url, model}})
  ...
  # Check if the error is a transient database error.
  ...
  @doc """
  Terminates the BatchBuilder for a specific batch if it is still the active
  process for that `{url, model}` key.
  """
  def terminate_for_batch(%{id: batch_id, url: url, model: model}) do
    case Registry.lookup(Batcher.Batching.Registry, {url, model}) do
      [{pid, _}] ->
        terminate_if_batch_matches(pid, batch_id)
  ...
  defp terminate_if_batch_matches(pid, batch_id) when is_pid(pid) do
    with true <- Process.alive?(pid),
         {:ok, %{batch_id: active_batch_id}} <- safe_get_state(pid),
         true <- active_batch_id == batch_id do
      try do
        GenServer.call(pid, :terminate, 5_000)
      catch
        :exit, _ -> :ok
      end
    else
      ...
      {:ok, batch} ->
        ...
        case Batcher.Batching.start_batch_upload(batch) do
          {:ok, _updated_batch} ->
            ...
            Registry.unregister(Batcher.Batching.Registry, {state.url, state.model})
  ...
  defp via_tuple(url, model) do
    {:via, Registry, {Batcher.Batching.Registry, {url, model}}}
  end
